...
if os.path.isfile("training_labels.dat") and os.path.isfile("training_features_matrix.dat") and os.path.isfile("testing_labels.dat") and os.path.isfile("testing_features_matrix.dat"):
    ...
    training_labels = [0] * len(training_spammers_feature_matrix) + [1] * len(training_legit_feature_matrix)
    ...
    testing_spammers = import_user_data("Testing_data/spammers.txt", "Testing_data/spammers_tweets.txt")
    calculate_features(testing_spammers)
    testing_spammers_feature_matrix = build_feature_matrix(testing_spammers)

    testing_legit = import_user_data("Testing_data/legitimate_users.txt", "Testing_data/legitimate_users_tweets.txt")
...
#         for user_info in all_user_info:
#             user_id = float(re.split(r'\t+', user_info)[0])
#             name_length = float(re.split(r'\t+', user_info)[6])  # Name length
#             description_length = float(re.split(r'\t+', user_info)[7][:-1])  # Bio length
#             numb_tweets = float(re.split(r'\t+', user_info)[5])  # Number of Tweets
#             numb_followings = float(re.split(r'\t+', user_info)[3])  # Number of Followings
#             numb_followers = float(re.split(r'\t+', user_info)[4])  # Number of Followers
#             if numb_followings != 0:
#                 ratio_wer_ing = numb_followers / numb_followings
#             else:
#                 ratio_wer_ing = 1
#             training_dict[user_id] = [name_length, description_length, numb_tweets, ratio_wer_ing]
#
#     with open(tweet_file) as f:
#         all_user_info = f.readlines()
#         for user_info in all_user_info:
#             user_id = float(re.split(r'\t+', user_info)[0])
#             if user_id in training_dict:
#                 tweet = re.split(r'\t+', user_info)[2]
#                 count_http = tweet.count("http:")
#                 if len(training_dict[user_id]) > 4:
#                     training_dict[user_id][4] += count_http
#                 else:
...
#             if user_id in training_dict:
#                 tweet = re.split(r'\t+', user_info)[2]
#                 count_at = tweet.count("@")
#                 if len(training_dict[user_id]) > 6:
#                     training_dict[user_id][6] += count_at
#                 else:
#                     listt = training_dict[user_id]
...
#
#
# training_features_spammer = make("Training_data/spammers.txt", "Training_data/spammers_tweets.txt")
# training_features_legit = make("Training_data/legitimate_users.txt", "Training_data/legitimate_users_tweets.txt")
# train_labels = [0] * len(training_features_spammer) + [1] * len(training_features_legit)
# training_features = training_features_spammer + training_features_legit
# x = np.array(training_features)
# norm_training_features = x / x.max(axis=0)
#
#
# testing_features_spammer = make("Testing_data/spammers.txt", "Testing_data/spammers_tweets.txt")
# testing_features_legit = make("Testing_data/legitimate_users.txt", "Testing_data/legitimate_users_tweets.txt")
# test_labels = [0] * len(testing_features_spammer) + [1] * len(testing_features_legit)
# testing_features = testing_features_spammer + testing_features_legit
# y = np.array(testing_features)
